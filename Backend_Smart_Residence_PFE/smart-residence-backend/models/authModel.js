...
class AuthModel {
    ...
    static async getSecurityQuestionByEmail(email) {
        const [rows] = await db.execute('SELECT security_question FROM `User` WHERE email = ?', [email]);
        return rows[0];
    }

    static async updatePassword(email, newHashedPassword) {
        return db.execute('UPDATE `User` SET password = ? WHERE email = ?', [newHashedPassword, email]);
    }
    static async verifySecurityAnswer(email, answer) {
        const sql = `
            SELECT * FROM \`User\` 
            WHERE email = ? 
            AND LOWER(TRIM(securityAnswer)) = LOWER(TRIM(?))
        `;
        const [rows] = await db.execute(sql, [email, answer]);
        return rows[0]; 
    }
}
