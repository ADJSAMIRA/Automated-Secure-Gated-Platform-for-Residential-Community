...
exports.likePostToggle = async (req, res) => {
    ...
    const id_user = req.body.id_user || req.body.userId || req.body.user_id || req.body.author_id;
    ...
    try {
        ...
        if (result.action === 'liked') {
            const postInfo = await PostModel.getPostAuthorAndLikerName(postId, id_user);

            if (postInfo.length > 0) {
                const postAuthorId = postInfo[0].author_id;
                const likerName = postInfo[0].likerName;

                if (postAuthorId !== parseInt(id_user)) {
