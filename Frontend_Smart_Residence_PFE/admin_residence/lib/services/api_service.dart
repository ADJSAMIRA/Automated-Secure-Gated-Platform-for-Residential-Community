...
class ApiService {
  //link 
  static const String baseUrl = "https://blast-extremely-brighton-entries.trycloudflare.com";
...
static Future<http.Response> getCommunityFeed() async {
    return await http.get(Uri.parse('$baseUrl/api/social/feed'));
  }
  ...
  static Future<http.Response> getPostComments(String postId) async {
    return await http.get(Uri.parse('$baseUrl/api/social/posts/$postId/comments'));
  }

  
  static Future<http.Response> addPostComment(String postId, String authorId, String text) async {
    return await http.post(
      Uri.parse('$baseUrl/api/social/posts/$postId/comments'),
      headers: {"Content-Type": "application/json"},
...
static Future<http.Response> likePostToggle(String postId, String userId) async {
  ...
  final String url = '$baseUrl/api/social/posts/$postId/like?userId=$userId&user_id=$userId&author_id=$userId';
...
static Future<List<dynamic>> getAccessLogs(String filterType) async {
  try {
   
    final url = Uri.parse('$baseUrl/access-logs');
    ...
    if (response.statusCode == 200) {
      ...
      else if (resBody is Map && resBody.containsKey('data')) {
        return resBody['data'] ?? [];
      } else if (resBody is Map && resBody.containsKey('logs')) {
        return resBody['logs'] ?? [];
      }
      
      return [];
    } else {
      print("Server Error [${response.statusCode}]: Failed to fetch access logs");
      return [];
    }
  } catch (e) {
    print("Network Exception inside getAccessLogs: $e");
    return [];
  }
}
//resat password
static Future<http.Response> getSecurityQuestion(String email) async {
  try {
    ...
    final url = Uri.parse('$cleanBaseUrl/api/auth/forgot-password/get-question');
    ...
    print("❌ Exception in getSecurityQuestion: $e");
    rethrow;
  }
}

static Future<http.Response> resetPasswordWithQuestion(String email, String securityAnswer, String newPassword) async {
  try {
    final cleanBaseUrl = baseUrl.endsWith('/') ? baseUrl.substring(0, baseUrl.length - 1) : baseUrl;
    final url = Uri.parse('$cleanBaseUrl/api/auth/forgot-password/reset');
    ...
    print("❌ Exception in resetPasswordWithQuestion: $e");
    rethrow;
  }
}
}